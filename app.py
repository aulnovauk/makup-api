...
class WebMakeupEngine:
    ...
    Thread safety: MediaPipe FaceMesh is NOT thread-safe.
    ...
    def __init__(self) -> None:
        try:
            ...
            self._static_mesh = mp.solutions.face_mesh.FaceMesh(
            ...
            log.error(f"MediaPipe not installed: {e}")
            raise
    ...
    def __del__(self) -> None:
        try:
            ...
            if hasattr(self, "_static_mesh"):
                self._static_mesh.close()
        except Exception:
            pass
    ...
    def _visible(lm: list, indices: list, thresh: float = 0.45) -> bool:
        return all((lm[i].visibility or 1.0) >= thresh for i in indices[:3])
    ...
    def render(self, frame: np.ndarray, params: dict,
        ...
        mesh = self._static_mesh if static else self._face_mesh
        ...
        for face_lm in results.multi_face_landmarks:
            ...
            if params.get("foundation", {}).get("enabled"):
                ...
                if self._visible(lm, FACE_OVAL):
                    ...
                    if fd.get("smooth"):
                        smooth_frame = cv2.bilateralFilter(frame, 9, 55, 55)
                        mask_f = np.zeros(frame.shape[:2], dtype=np.uint8)
                        cv2.fillPoly(mask_f, [face_pts], 255)
                        mf = mask_f.astype(np.float32) / 255.0 * 0.65
                        mf3 = cv2.merge([mf, mf, mf])
                        output = np.clip(
                            frame.astype(np.float32) * (1 - mf3) +
